...
Do these two Whatman cards exist in LOKI?
```{r}
sum(grepl(pattern = 2051, x = loki_tissue_summer$DNA_TRAY_CODE))
sum(grepl(pattern = 2240, x = loki_tissue_summer$DNA_TRAY_CODE))
sum(grepl(pattern = 3900, x = loki_tissue_summer$DNA_TRAY_CODE))
```
...
As for fish 205106, it looks like we only have 9 in LOKI from that Whatman card, how many did we want to extract?
```{r}
summer_torun_asl[grep(pattern = 2051, x = summer_torun_asl$`Dna Specimen No`), ] %>% 
  arrange(`Dna Specimen No`)
```

Looks like we wanted 7 total, so we can just drop 205106 and replace with another one.
```{r}
summer_torun_asl_mod <- summer_torun_asl_mod %>% 
  filter(`Dna Specimen No` != 205106) %>% 
  bind_rows(filter(asl_summer, `Dna Specimen No` == 205105))
```

As for fish 224010, it looks like we only have 8 in LOKI from that Whatman card, how many did we want to extract?
```{r}
summer_torun_asl[grep(pattern = 2240, x = summer_torun_asl$`Dna Specimen No`), ] %>% 
  arrange(`Dna Specimen No`)
```

Does 224005 exist in LOKI?
```{r}
grep(pattern = "2240", x = loki_tissue_summer$dna_specimen_no, value = TRUE)
```


Looks like we wanted 8 total, so we can just drop 224010 and replace with another one.
```{r}
summer_torun_asl_mod <- summer_torun_asl_mod %>% 
  filter(`Dna Specimen No` != 224010) %>% 
  bind_rows(filter(asl_summer, `Dna Specimen No` == 224005))
```
...
loki_dups <- loki_tissue_summer %>% 
  count(dna_specimen_no) %>% 
  filter(n > 1) %>% 
  pull(dna_specimen_no)

loki_tissue_summer %>% 
  filter(dna_specimen_no %in% loki_dups)
```


Shit, I need to fix these LOKI duplicate dna_specimen_no before doing anything else.
```{r}
summer_torun_asl_mod %>% 
  filter(`Dna Specimen No` %in% loki_dups)
```
...
Shit, looks like one fish selected for extraction have the same Dna Specimen No, which are they?
```{r}
summer_torun_asl_mod %>% 
  count(`Dna Specimen No`) %>% 
  filter(n > 1)
...
```{r}
summer_torun_asl_mod %>% 
  filter(`Dna Specimen No` == 420402)
```

Any more fish on these cards?
```{r}
summer_torun_asl_mod[grep(pattern = 4204, x = summer_torun_asl_mod$`Dna Specimen No`), ]
```

After reviewing Iris' sample summary sheets, 445601 is the Summer Ret 1 fish from 171, Juneau, SW 47. The Summer Ret 2 fish from 171, Juneau, SW10 should be 645601.
...
And everything still in LOKI?
```{r}
all(summer_torun_asl_mod$`Dna Specimen No` %in% loki_tissue_summer$dna_specimen_no)
```
