...
spring_torun_asl %>% 
  filter(`Dna Specimen No` %in% missing_fish) %>% 
  select(`Stat Area`, Month, `Stat Week`, `Dna Specimen No`)
```

Do these two Whatman cards exist in LOKI?
```{r}
sum(grepl(pattern = 2432, x = loki_tissue_spring$WGC_4digit))
sum(grepl(pattern = 2240, x = loki_tissue_spring$WGC_4digit))
sum(grepl(pattern = 3900, x = loki_tissue_spring$WGC_4digit))
sum(grepl(pattern = 3894, x = loki_tissue_spring$WGC_4digit))
```
...
As for fish 243207, it looks like we only have 9 in LOKI from that Whatman card, how many did we want to extract?
```{r}
spring_torun_asl[grep(pattern = 243207, x = spring_torun_asl$`Dna Specimen No`), ] %>% 
  arrange(`Dna Specimen No`)
```
...
spring_torun_asl_mod <- spring_torun_asl_mod %>% 
  filter(`Dna Specimen No` != 243207) %>% 
  bind_rows(filter(asl_spring, `Dna Specimen No` == 243206))
```

As for fish 224003, it looks like we only have 9 in LOKI from that Whatman card, how many did we want to extract?
```{r}
spring_torun_asl[grep(pattern = 2240, x = spring_torun_asl$`Dna Specimen No`), ] %>% 
  arrange(`Dna Specimen No`)
```
...
spring_torun_asl_mod <- spring_torun_asl_mod %>% 
  filter(`Dna Specimen No` != 224003) %>% 
  bind_rows(filter(asl_spring, `Dna Specimen No` == 224001))
```

As for fish 389409, it looks like we only have 9 in LOKI from that Whatman card, how many did we want to extract?
```{r}
spring_torun_asl[grep(pattern = 3894, x = spring_torun_asl$`Dna Specimen No`), ] %>% 
  arrange(`Dna Specimen No`)
```
...
spring_torun_asl_mod <- spring_torun_asl_mod %>% 
  filter(`Dna Specimen No` != 389409) %>% 
  bind_rows(filter(asl_spring, `Dna Specimen No` == 389407))
```
...
### Write
```{r}
write_csv(spring_torun_extraction, path = "../Extraction Lists/Summer_Extraction.csv")
